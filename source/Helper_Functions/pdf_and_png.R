...
pdf_and_png <- function(p, outputDir, filename, pdfWidth = 7, pdfHeight = 7, splitDirectories = F, SAVE_RDS = F, SKIP_IF_EXISTS = F, scale = 1, units = "in") {
    if (splitDirectories) {
    ...
    if (!SKIP_IF_EXISTS || !file.exists(png_path)) {
        ggsave(png_path, plot = p, width = pdfWidth, height = pdfHeight, dpi = 300, limitsize = F, scale = scale, units = units)
    } else {
        print(paste0("SKIPPING: ", png_path))
    }

    if (!SKIP_IF_EXISTS || !file.exists(pdf_path)) {
        ggsave(pdf_path, plot = p, width = pdfWidth, height = pdfHeight, dpi = 300, limitsize = F, scale = scale, device = cairo_pdf, units = units)
    } else {
