...
library(ggthemes)
library(slingshot)
# library(BUSpaRse) No longer needed
library(tidyverse)
library(tidymodels)
library(scales)
library(viridis)
...
source("write_csv_append_md.r")

# Increase saturation of an RGB color vector
increase_saturation <- function(rgb_color, saturation_increase = 0.2) {
    # Convert RGB to HSV
    hsv_color <- rgb2hsv(col2rgb(rgb_color))

    # Increase saturation (clamp to max of 1)
    hsv_color[2, ] <- pmin(1, hsv_color[2, ] + saturation_increase)

    # Convert back to RGB
    new_color <- hsv(hsv_color[1, ], hsv_color[2, ], hsv_color[3, ])
    if (!is.null(names(rgb_color))) {
        names(new_color) <- names(rgb_color)
    }
    return(new_color)
}
...
runSlingshot <- function(
    ...
    if (file.exists(file.path(output, paste0("sce_", nameMod, ".rds"))) & !REBUILD_TRAJECTORY) {
        ...
        seurat_object.sce <- slingshot(seurat_object.sce,
            clusterLabels = "ident",
            reducedDim = slingshotDim, allow.breaks = T, start.clus = startCluster, end.clus = endCluster, approx_points = 150
        )
