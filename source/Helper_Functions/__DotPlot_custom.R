    ...
    if (!USE_PATCHWORK_INSTEAD_OF_FACETS) {
        ...
        if (!DO_TILE) {
                ...
                for (x in unique(feature.groups_customfacet)) {
                    if (x == "CUSTOMCOLOR") {
                        ...
                        plotList[[plotNum]] <- ggplot(data = data.plot.tmp, mapping = aes_string(x = "color_x_plot", y = "id")) +
                            ...
                            geom_text(label = "▶", family = "DejaVu Sans", mapping = aes(color_x_plot, y = id, color = id), size = 5, vjust = 0.33, show.legend = F) +
                        ...
                        plotList[[plotNum]] <- ggplot(data = data.plot.tmp, mapping = aes_string(x = "features.plot", y = "id")) +
                            geom_point(mapping = aes_string(size = "pct.exp", fill = color.by), shape = 21, colour = "grey", stroke = 0.33) +
                            theme_classic(base_size = base_size) +
                            ...
                            scale.func(name = "Percent\nExpression", labels = c("0%", "25%", "50%", "75%", "100%"), breaks = c(0, 25, 50, 75, 100), range = c(0, dot.scale), limits = c(0, 100)) +
                        ...
                        plotList[[plotNum]] <- plotList[[plotNum]] + theme(
                            strip.clip = "off",
            ...
            plot <- patchwork::wrap_plots(plotList, ncol = length(unique(feature.groups_customfacet)), byrow = T, guides = "collect", axes = "collect", axes_titles = "collect") &
                theme(legend.box = box_plot_orientation, legend.spacing = unit(4, "pt"), legend.spacing.x = unit(4, "pt"), legend.key.spacing.y = unit(1, "pt"), legend.title = element_text(margin = margin(b = 2)), legend.box.spacing = unit(2, "pt"))
        }
            ...
            for (y in unique(idents.groups)) {
                for (x in unique(feature.groups_customfacet)) {
                    if (x == "CUSTOMCOLOR") {
                        ...
                        plotList[[plotNum]] <- ggplot(data = data.plot.tmp, mapping = aes_string(x = "color_x_plot", y = "id")) +
                            geom_raster(fill = "white", mapping = aes_string(x = "color_x_plot", y = "id")) +
                            geom_text(label = "▶", family = "DejaVu Sans", mapping = aes(color_x_plot, y = id, color = id), size = 10, vjust = 0.5, show.legend = F) +
                            scale_color_manual(values = CLUSTER_COLORS) +
                            theme_classic(base_size = base_size) +
                            # scale_x_discrete(expand = c(0, 0)) +
                            # scale_y_discrete(expand = c(0, 0)) +
                        ...
                        plotList[[plotNum]] <- ggplot(data = data.plot.tmp, mapping = aes_string(x = "features.plot", y = "id")) +
                            ...
                            scale_fill_gradient2(
                                ...
                                high = "#EA7FA3",
                                limits = range(c(-2, 2)),
                                guide = guide_colorbar(title = "Average Expression", barheight = unit(1.34, "cm"), barwidth = unit(0.3, "cm"), ticks.linewidth = 0.5)
                                # limits = range(c(min(data.plot[, color.by]), max(data.plot[, color.by])))
                            ) +
                            coord_fixed()
                        if (y != last_ident) {
